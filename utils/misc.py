...
def populate_textNtonesarray(fname, feats_dir, feats_dict):
    text_array = []
    tone_array = []
    f = open(fname)
    for line in f:
        ...
        for feat in feats:
            #print(feat) 
            text, tone = feat.split('_')[0], feat.split('_')[1]
            text_array.append(feats_dict[text])
            tone_array.append(feats_dict[tone])
    text_array = np.array(text_array)
    tone_array = np.array(tone_array) 
    return text_array, tone_array
...
def collate_fn_xspkmely(batch):
    ...
    #sys.exit()
    r = 5
    tones = [x[1] for (x,_,_) in batch]
    texts = [x[0] for (x, _,_) in batch]
    #print(tones)
    assert len(tones[0]) == len(texts[0])
    #sys.exit()

    input_lengths = [len(x) for x in texts]
    max_input_len = np.max(input_lengths) + 1
    # Add single zeros frame at least, so plus 1
    ...
    a = np.array([_pad(x, max_input_len) for x in texts], dtype=np.int)
    x_batch = torch.LongTensor(a)

    a_tone = np.array([_pad(x, max_input_len) for x in tones], dtype=np.int)
    tone_batch = torch.LongTensor(a_tone)
    ...
    return x_batch, tone_batch, input_lengths, mel_batch, y_batch
...
def init_defaultdict_fromdict(dict):

    def_dict = defaultdict(lambda: len(dict))
...
def populate_subtextarray_english(fname, feats_dir, char_ids, wordfiles_dir):
    ...
    for line in f:
      content = line.split('\n')[0].strip().split()
      word = content[0].lower().replace("'","").replace('[','').replace(']','')
      #print(word)
