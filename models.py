...
class TacotronOneSeqwiseTones(TacotronOneSeqwise):
...
class TacotronOneSeqwiseTones_linearconvembedding(TacotronOneSeqwiseTones):

    def __init__(self, n_vocab, embedding_dim=256, mel_dim=80, linear_dim=1025,
                 r=5, padding_idx=None, use_memory_mask=False):
        super(TacotronOneSeqwiseTones_linearconvembedding, self).__init__(n_vocab, embedding_dim=256, mel_dim=80, linear_dim=1025,
                 r=5, padding_idx=None, use_memory_mask=False)

        self.toneNchar2embedding = nn.Conv1d(embedding_dim*2,embedding_dim, kernel_size=3, padding=1)
...
class TacotronOneSeqwiseqF0s(TacotronOneSeqwise):
    ...
    def __init__(self, n_vocab, embedding_dim=256, mel_dim=80, linear_dim=1025,
        ...
        super(TacotronOneSeqwiseqF0s, self).__init__(n_vocab, embedding_dim=256, mel_dim=80, linear_dim=1025,
                 r=5, padding_idx=None, use_memory_mask=False)
        #self.encoder = Encoder_TacotronOneSeqwise(embedding_dim)
        self.conv_qf0 = nn.Conv1d(1,4,kernel_size=3,padding=1)
        self.toneNchar2embedding = SequenceWise(nn.Linear(embedding_dim+4,embedding_dim))
    ...
    def forward_nomasking(self, inputs, tones, targets=None, input_lengths=None):
        ...
        inputs = self.embedding(inputs)
        #print("Shape of tones: ", tones.shape )
        inputs_tones = self.conv_qf0(tones.float().unsqueeze(1)).transpose(1,2)
        inputs = torch.cat([inputs, inputs_tones], dim=-1)
        inputs = self.toneNchar2embedding(inputs)
